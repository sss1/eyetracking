...
for subject_id in subject_id_list:
    new_trendline, _ = asc.read_and_analyze_file(maindir, "AllSame", subject_id, filter_threshold = filter_threshold)
    if new_trendline != None and len(new_trendline) > 0:
        trendline_list_same.append(new_trendline)
    new_trendline, _ = asc.read_and_analyze_file(maindir, "AllDiff", subject_id, filter_threshold = filter_threshold)
    if new_trendline != None and len(new_trendline) > 0:
        trendline_list_diff.append(new_trendline)
    new_trendline, _ = asc.read_and_analyze_file(maindir, "0Distractors", subject_id, filter_threshold = filter_threshold)
    if new_trendline != None and len(new_trendline) > 0:
...
# TODO: THIS LEGEND IS INCORRECT IF NOT ALL SUBJECTS HAVE VALID DATA,
